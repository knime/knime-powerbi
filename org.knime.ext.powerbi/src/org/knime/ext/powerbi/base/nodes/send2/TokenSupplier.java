...
/**
 * A wrapper for {@link OAuth2Credential}s that remembers the token and updates it when it is not valid anymore for
 * longer than 1min.
 *
 * @author Benjamin Wilhelm, KNIME GmbH, Konstanz, Germany
 */
class TokenSupplier implements AuthTokenProvider {
    ...
    TokenSupplier(final OAuth2Credential credential) {
    ...
    private void refreshTokenIfNecessary() throws IOException {
        if (needsUpdate(m_token)) {
            refreshToken();
        }
    }
    ...
    private synchronized void refreshToken() throws IOException {
        ...
        if (needsUpdate(m_token)) {
            m_token = m_credential.getAccessToken();
        }
    }

    /** @return true if the token is null or will expire soon */
    private static boolean needsUpdate(final OAuth2AccessToken token) {
        return token == null
            || token.getAccessTokenExpiresAt().isBefore(Instant.now().plusMillis(MIN_TOKEN_VALID_TIME));
    }
}
