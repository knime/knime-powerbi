...
public class AzureADAuthenticator implements Authenticator {
    ...
    private SwingWorkerWithContext<AzureADAuthentication, Void> m_swingWorker;

    private AzureADAuthentication m_auth;
    ...
    public void authenticate() {
        m_swingWorker = new SwingWorkerWithContext<AzureADAuthentication, Void>() {

            private Future<AzureADAuthentication> m_futureAuth;

            @Override
            protected AzureADAuthentication doInBackgroundWithContext() throws Exception {
                m_futureAuth = AzureADAuthenticationUtils.authenticate(m_scope);
     ...
     */
    public void setAuthentication(final AzureADAuthentication auth) {
        m_auth = auth;
        if (auth != null) {
            updateState(AuthenticatorState.AUTHENTICATED);
        } else {
